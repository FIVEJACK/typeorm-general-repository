...
it('should return using simple pagination (getMany)', async () => {
  ...
  expect(result).toEqual({current_page: 1, data: [{created_at: mockDateNow, id: 1, name: 'itemku', updated_at: mockDateNow}], http_status_code: 200, item_per_page: 10, message: 'Success', next_page: 2, prev_page: null, success: true});
});
